class ExpensesDatatable
  ...
  def edit_link(expense)
    link_to fa_icon('pencil', text: 'edit'), [:edit, :api, :v1, expense], remote: true, id: "edit_expense_#{expense.id}", class: "edit"
  end

  def destroy_link(expense)
    link_to fa_icon('times', text: 'delete'), [:api, :v1, expense], method: :delete, remote: true, id: "destroy_expense_#{expense.id}", class: "destroy", data: { confirm: "Delete expense?" }
  end
end
